...
function subscribe(uri) {
  ...
  function connect(uri) {
    const events = new EventSource(uri); // object for listening to new messages.

    events.addEventListener("message", (ev) => {
      console.log("raw data", JSON.stringify(ev.data));
      console.log("decoded data", JSON.stringify(JSON.parse(ev.data))); // parse to JSON
      const msg = JSON.parse(ev.data);
      if (!("message" in msg) || !("room" in msg) || !("username" in msg))
        return;
      addMessage(msg.room, msg.username, msg.message, true); //append to DOM
    });
...
function init() {
  ...
  newMessageForm.addEventListener("submit", (e) => {
    ...
    if (STATE.connected) {
      // POST if connected so we can send messages!
      fetch("/message", {
        method: "POST",
